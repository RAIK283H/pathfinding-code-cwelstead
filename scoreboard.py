...
class Scoreboard:
    ...
    player_path_display = []
    player_average_distance_display = []
    ...
    def __init__(self, batch, group):
        ...
        for index, player in enumerate(config_data.player_data):
            ...
            average_distance_label = pyglet.text.Label("Avg Dist per Node:",
                                    x=0,
                                    y=0,
                                    font_name='Arial',
                                    font_size=self.font_size, batch=batch, group=group, color=player[2][colors.TEXT_INDEX])
            self.player_average_distance_display.append(
                (average_distance_label, player))
    ...
    def update_elements_locations(self):
        ...
        for index, (display_element, player) in enumerate(self.player_average_distance_display):
            display_element.x = config_data.window_width - self.stat_width
    ...
    def update_average_distance(self):
        for display_element, player_configuration_info in self.player_average_distance_display:
            for index, player_object in enumerate(global_game_data.player_objects):
                if player_object.player_config_data == player_configuration_info:
                    display_element.text = "Avg Dist per Node: " + str(int(player_object.distance_traveled) // len(global_game_data.graph_paths[index]))
